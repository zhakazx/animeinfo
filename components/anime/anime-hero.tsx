    ...
    <section className="relative h-[50vh] xs:h-[55vh] sm:h-[60vh] min-h-[400px] xs:min-h-[450px] sm:min-h-[500px] w-full overflow-hidden">
      ...
      <div className="relative h-full flex items-end justify-center">
        <div className="container mx-auto px-3 xs:px-4 pb-6 xs:pb-8 sm:pb-12">
          <div className="flex flex-col md:flex-row gap-4 xs:gap-6 sm:gap-8 justify-center">
            {/* Poster Image */}
            ...
            <div className="flex-1 space-y-3 xs:space-y-4 text-center md:text-left">
              ...
              <div className="flex items-center justify-center md:justify-start gap-1.5 xs:gap-2 flex-wrap">
                {anime.airing && (
                  <Badge
                    variant="destructive"
                    className="bg-red-500/90 text-xs xs:text-sm"
                  >
                    Currently Airing
                  </Badge>
                )}
                ...
                <div className="flex flex-wrap justify-center md:justify-start gap-1.5 xs:gap-2">
                  {anime.genres.slice(0, 4).map((genre) => (
                    <Badge
                      key={genre.mal_id}
                      variant="outline"
                      className="bg-background/50 text-xs xs:text-sm"
                    >
                      {genre.name}
                    </Badge>
                  ))}
              ...
              <div className="flex flex-col xs:flex-row gap-2 xs:gap-3 sm:gap-4 pt-2 xs:pt-4">
                {anime.trailer?.youtube_id && (
                  <Button
                    size="sm"
                    asChild
                    className="bg-primary hover:bg-primary/90 xs:size-default"
                  >
                    <Link href="#trailer">
                ...
                {anime.url && (
                  <Button
                    ...
                    variant="outline"
                    asChild
                    className="bg-background/50 hover:bg-background/70 xs:size-default"
                  >
                    <a
                      href={anime.url}
                      target="_blank"
